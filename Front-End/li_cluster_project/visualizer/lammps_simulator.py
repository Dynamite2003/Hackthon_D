...
try:
    ...
    print("Warning: ASE not found. Please install it using 'pip install ase'")
...
class LAMMPSSimulator:
    ...
    def create_lammps_data_file(self, coords, data_file_path):
        ...
        # Calculate box size to accommodate all coordinates with padding
        if num_atoms > 0:
...
"""
        ...
        # Add atom coordinates (preserve original coordinates)
        for i, (x, y, z) in enumerate(coords, 1):
            data_content += f"{i} 1 {x:.6f} {y:.6f} {z:.6f}\n"
    ...
    def create_lammps_input_script(self, data_file, output_xyz, input_script_path, target_pdb_path=None):
        ...
        if target_coords_file and os.path.exists(target_coords_file):
            # 使用基于弹簧的TMD模拟 
            script_content = f"""# TMD simulation using spring forces toward target
units metal
atom_style atomic
boundary f f f
...
"""
            ...
            try:
                target_coords = self.parse_pdb_coordinates(target_pdb_path)
                # 为每个原子创建单独的组（先不加弹簧）
                for i, (x, y, z) in enumerate(target_coords, 1):
                    script_content += f"group atom{i} id {i}\n"
...
dump traj_dump all xyz 50 {os.path.basename(output_xyz)}
...
"""
                for i, (x, y, z) in enumerate(target_coords, 1):
                    script_content += f"fix spring{i}_1 atom{i} spring tether ${{k_stage1}} {x:.6f} {y:.6f} {z:.6f} 0.0\n"
...
"""
                for i in range(1, len(target_coords) + 1):
                    script_content += f"unfix spring{i}_1\n"
                for i, (x, y, z) in enumerate(target_coords, 1):
                    script_content += f"fix spring{i}_2 atom{i} spring tether ${{k_stage2}} {x:.6f} {y:.6f} {z:.6f} 0.0\n"
...
"""
                for i in range(1, len(target_coords) + 1):
                    script_content += f"unfix spring{i}_2\n"
                for i, (x, y, z) in enumerate(target_coords, 1):
                    script_content += f"fix spring{i}_3 atom{i} spring tether ${{k_stage3}} {x:.6f} {y:.6f} {z:.6f} 0.0\n"
...
"""
                for i in range(1, len(target_coords) + 1):
                    script_content += f"unfix spring{i}_3\n"
                for i, (x, y, z) in enumerate(target_coords, 1):
                    script_content += f"fix spring{i}_4 atom{i} spring tether ${{k_final}} {x:.6f} {y:.6f} {z:.6f} 0.0\n"
...
"""
                for i in range(1, len(target_coords) + 1):
                    script_content += f"unfix spring{i}_4\n"
...
"""
                ...
                print(f"Error setting up TMD springs: {e}")
                # 回退到标准MD
                script_content = self._get_standard_md_script(data_file, output_xyz)
        else:
            # 标准MD模拟
            script_content = self._get_standard_md_script(data_file, output_xyz)
    ...
    def _get_standard_md_script(self, data_file, output_xyz):
        """生成标准MD脚本"""
        return f"""# Standard LAMMPS MD simulation
units metal
atom_style atomic
boundary f f f
...
"""
    ...
    def _generate_fallback_trajectory(self, initial_coords, target_pdb_path, output_xyz):
        ...
        try:
            ...
            # Write XYZ trajectory using ASE if available, otherwise manual write
            try:
                ase_trajectory = []
                ...
                # Manual XYZ write as fallback
                with open(output_xyz, 'w') as f:
                    for frame_coords in trajectory:
                        f.write(f"{len(frame_coords)}\n")
                        f.write("Generated trajectory frame\n")
                        for i, (x, y, z) in enumerate(frame_coords):
                            f.write(f"Li {x:.6f} {y:.6f} {z:.6f}\n")
