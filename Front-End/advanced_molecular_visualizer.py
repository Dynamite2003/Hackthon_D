...
class AdvancedMolecularVisualizer:
    """
    高级分子可视化模块 - 使用 py3Dmol 进行高质量3D渲染
    """
    def __init__(self):
        pass
    ...
    def visualize_cluster(self, base_positions, predicted_sites=None, 
        ...
        view.setStyle({'sphere': {'scale': 0.8}})
        ...
        view.setStyle({'elem': 'Li'}, {'sphere': {'color': '#CC80FF'}})
        for elem, color in color_map.items():
            view.setStyle({'elem': elem}, {'sphere': {'color': color, 'opacity': 0.9}})
        ...
        if save_path:
            with open(save_path, 'w') as f:
                f.write(html)
            print(f"交互式py3Dmol图已保存到: {save_path}")
